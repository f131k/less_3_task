...
pub trait Lexer {
    fn tokenize<'a>(&self, input: &'a mut str) -> Result<TokenList, &str>;
}

pub struct EmptyLexer {}

impl Lexer for EmptyLexer {
    fn tokenize<'a>(&self, _: &'a mut str) -> Result<TokenList, &str> {
        Err("Empty")
    }
}
...
impl Lexer for RegexpLexer {
    fn tokenize<'a>(&self, input: &'a mut str) -> Result<TokenList, &str> {
        ...
        while !target_string.is_empty() && !error {
            ...
            for tok in &self.knows_tokens {
                ...
                if let Some(captions) = rgx.captures(&target_string) {
                    ...
                    target_string = target_string.trim_start_matches(value).to_string();
                    break;
                }
            }
        ...
        dbg!("Разбор на лексемы завершен");
